...
The human protein degree was computed by `NetworkX`, and the computed result was stored in `./demo_dataset/VirusHumanSimiAndDegree/node_feat_human.tsv`

### Viral mimicry of human protein interactions
We computed similarity between viral proteins and neighbor proteins with `./script/step1_get_seqsimi_diamod.py` and integrated interaction scores between neighbor proteins and human protein with `./script/step2_0_ppi_prepare.py`(for preparing protein-protein pairs) & `./script/step2_sum_seqsimi_allhv_optimize.py`
