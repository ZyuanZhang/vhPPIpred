...
def train_model_repeat_cv(repeats = 5, k = 5):
    # 加载数据
    dt_kmer_count = pd.read_csv("../case_scripts_by_zhangzhiyuan/cases_res_files/xy_5kmer.csv", sep="\t", header=0, index_col=0)
    X = np.array(dt_kmer_count.iloc[:, 1:-2])  # Features
    ...
    df_logs.to_csv("./tmp/loss_mlp_vgenome_5kmer.csv", index=False)



def train_model_repeat_cv_eval(repeats=5, k=5, num_epochs = 50, learning_rate = 0.005):
    # 加载数据
    dt_kmer_count = pd.read_csv("../case_scripts_by_zhangzhiyuan/cases_res_files/xy_5kmer.csv", sep="\t", header=0, index_col=0)
    X = np.array(dt_kmer_count.iloc[:, 1:-2])  # Features
    y = np.array(dt_kmer_count.iloc[:, -2])  # Target labels
    ...
    df_all.to_csv("./tmp/cv_res_mlp_vgenome_5kmer.csv", index=False)
...
def test_model_on_independent_set(repeats = 5, num_epochs = 50, learning_rate = 0.005):
    # 加载数据
    dt_kmer_count = pd.read_csv("../case_scripts_by_zhangzhiyuan/cases_res_files/xy_5kmer.csv", sep="\t", header=0, index_col=0)
    X = np.array(dt_kmer_count.iloc[:, 1:-2])  # Features
    y = np.array(dt_kmer_count.iloc[:, -2])  # Target labels
    ...
    df_test.to_csv("./tmp/test_res_mlp_vgenome_5kmer.csv", index=False)
