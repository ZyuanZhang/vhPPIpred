...
def train_model_repeat_cv(repeats = 5, k = 5):
    # 加载数据
    dt_kmer_count = pd.read_csv("../case_scripts_by_zhangzhiyuan/cases_res_files/xy_4kmer.csv", sep="\t", header=0, index_col=0)
    with open ("/data/150T/databases/help_zhangzhiyuan/PredictAllHumanVirusPpiDatasetEmbedCase3/v214_genome_embed_single.json","r") as inF:
        dict_vtaxid_gembed = json.load(inF)
    dt_kmer_count_X = []
    for i in range(dt_kmer_count.shape[0]):
        vtaxid = str(dt_kmer_count["id"][i])
        dt_kmer_count_X.append(dict_vtaxid_gembed[vtaxid])
    X = np.array(dt_kmer_count_X)
    y = np.array(dt_kmer_count.iloc[:, -2])  # Target labels
    ...
    df_logs.to_csv("./tmp/loss_mlp_vprot_dnabert.csv", index=False)


def train_model_repeat_cv_eval(repeats=5, k=5, num_epochs = 40, learning_rate = 0.005):
    # 加载数据
    dt_kmer_count = pd.read_csv("../case_scripts_by_zhangzhiyuan/cases_res_files/xy_4kmer.csv", sep="\t", header=0, index_col=0)
    with open ("/data/150T/databases/help_zhangzhiyuan/PredictAllHumanVirusPpiDatasetEmbedCase3/v214_genome_embed_single.json","r") as inF:
        dict_vtaxid_gembed = json.load(inF)
    ...
    df_all.to_csv("./tmp/cv_res_mlp_vprot_dnabert.csv", index=False)
...
def test_model_on_independent_set(repeats = 5, num_epochs = 40, learning_rate = 0.005):
    # 加载数据
    dt_kmer_count = pd.read_csv("../case_scripts_by_zhangzhiyuan/cases_res_files/xy_4kmer.csv", sep="\t", header=0, index_col=0)
    with open ("/data/150T/databases/help_zhangzhiyuan/PredictAllHumanVirusPpiDatasetEmbedCase3/v214_genome_embed_single.json","r") as inF:
        dict_vtaxid_gembed = json.load(inF)
    ...
    df_test.to_csv("./tmp/test_res_mlp_vprot_dnabert.csv", index=False)
