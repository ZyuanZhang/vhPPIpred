...
def train_model_repeat_cv_eval(repeats=5, k=5, rf_n_estimators=500, rf_max_depth=9):
    ...
    df_all.to_csv("./tmp/cv_res_rf_vprot_prott5.csv", index=False)
...
def test_model_on_independent_set(repeats=5, rf_n_estimators=500, rf_max_depth=9):
    # 加载数据
    dt_kmer_count = pd.read_csv("../case_scripts_by_zhangzhiyuan/cases_res_files/xy_4kmer.csv", sep="\t", header=0, index_col=0)
    with open ("/data/150T/databases/help_zhangzhiyuan/PredictAllHumanVirusPpiDatasetEmbedCase3/v214_proteome_embed_single.json","r") as inF:
        dict_vtaxid_gembed = json.load(inF)
    ...
    df_test.to_csv("./tmp/test_res_rf_vprot_prott5.csv", index=False)
