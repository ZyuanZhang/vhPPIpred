...
def train_model_repeat_cv(repeats = 5, k = 5):
    # 加载数据
    df = pd.read_csv("../case3_virulence_pred_data_from_fengyang/ppi_xyzhang.csv", sep="\t", header=0, index_col=0)
    ...
    df_logs.to_csv("./tmp/loss_mlp_ppi_binary.csv", index=False)


def train_model_repeat_cv_eval(repeats=5, k=5, num_epochs = 15, learning_rate = 0.001):
    # 加载数据
    df = pd.read_csv("../case3_virulence_pred_data_from_fengyang/ppi_xyzhang.csv", sep="\t", header=0, index_col=0)
    X = np.array(df.iloc[:, 1:-2])
    y = np.array(df.iloc[:, -1])
    ...
    df_all.to_csv("./tmp/cv_res_mlp_ppi_binary.csv", index=False)
...
def test_model_on_independent_set(repeats = 5, num_epochs = 15, learning_rate = 0.001):
    # 加载数据
    df = pd.read_csv("../case3_virulence_pred_data_from_fengyang/ppi_xyzhang.csv", sep="\t", header=0, index_col=0)
    X = np.array(df.iloc[:, 1:-2])
    y = np.array(df.iloc[:, -1])
    ...
    df_test.to_csv("./tmp/test_res_mlp_ppi_binary.csv", index=False)
